...
export const App: FC = () => {
  ...
  const fetchRandomData = useCallback(async () => {
      const {data} = await axios.get<ChuckNorrisSearchResponseType>('https://api.chucknorris.io/jokes/search?query=test')
  ...
  return <>
     {data.result.map((data) => (<Card icon={data.icon_url} text={data.value} key={data.id}/>))}
  </>
}
